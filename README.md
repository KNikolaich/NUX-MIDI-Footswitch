...
- ESP32 DevKit / ESP32-WROOM-32 with two buttons and no display: The minimal sketch in `src/NUX-Pro-FS-MIDI-Client-ESP32-DevKit-Encoder` targets the MIGHTY PLUG PRO. The button on GPIO35 selects the next of seven presets, and the button on GPIO0 sends the currently selected preset over BLE-MIDI. It reports BLE/preset status through Serial Monitor at 115200 baud and requires no display library. GPIO35 needs an external pull-up because it has no internal pull-up; GPIO0 is a boot-strap pin and must not be held low during reset or power-up.

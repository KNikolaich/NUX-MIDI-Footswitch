...
- TTGO T-Display ESP32 with two built-in buttons: The sketch in `src/NUX-Pro-FS-MIDI-Client-TTGO-T-Display-Encoder` uses the built-in ST7789 135x240 TFT and targets the MIGHTY PLUG PRO. The built-in GPIO35 button selects the next of seven presets, and the built-in GPIO0 button sends the currently selected preset using MIDI Program Change. The large preset number is shown on the right side of the TFT; BLE status, button hints, battery voltage and an estimated battery percentage are shown on the left. Battery voltage is measured through GPIO34 while the TTGO divider is enabled on GPIO14; the percentage is an approximation from 3.30 V (empty) to 4.20 V (full). On connection, the sketch requests the current preset using the NUX private SysEx protocol and also listens for incoming Program Change messages. GPIO35 is input-only and has no internal pull-up; GPIO0 is a boot-strap pin and must not be held low during reset or power-up. This variant keeps GPIO1/GPIO3 available for Serial Monitor and requires the `TFT_eSPI` library with the TTGO T-Display setup selected.

...
- TTGO T-Display ESP32 with two built-in quick-preset buttons and a rotary encoder: The sketch in `src/NUX-Pro-FS-MIDI-Client-TTGO-T-Display-Encoder` uses the built-in ST7789 135x240 TFT and targets the MIGHTY PLUG PRO. GPIO35 immediately recalls the preset defined by `QUICK_PRESET_A` (default 1), and GPIO0 immediately recalls `QUICK_PRESET_B` (default 5). The external encoder uses CLK=GPIO25, DT=GPIO26 and SW=GPIO27; rotation selects any of the seven presets without sending it, and pressing SW sends the selected preset using MIDI Program Change. The large preset number is shown on the right side of the TFT; BLE status, quick-preset assignments, battery voltage and an estimated battery percentage are shown on the left. Battery voltage is measured through GPIO34 while the TTGO divider is enabled on GPIO14; the percentage is an approximation from 3.30 V (empty) to 4.20 V (full). On connection, the sketch requests the current preset using the NUX private SysEx protocol and also listens for incoming Program Change messages. GPIO35 is input-only and has no internal pull-up; GPIO0 is a boot-strap pin and must not be held low during reset or power-up. This variant keeps GPIO1/GPIO3 available for Serial Monitor and requires the `TFT_eSPI` library with the TTGO T-Display setup selected.
